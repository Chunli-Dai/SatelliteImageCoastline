function [Co]=CoastTile(tilefile,S,range,f,fdir);
% coastline from orthoimages;mosaicking of coastlines
% Requirements: Matlab R2018a for polyshape.m; -- not needed.
%               MIMC2 for horizontal coregistration;
% Versions:
% CoastTilev2.m: to save space: do not save all groups, just save the data that has the maximum repeats, and disgrad others.
% Version codec2: use probability to mask water body
% 
...
Me1=imerode(wm.z, ones(width/resrc)); %Undesirably erode the box edge if sx (shapefile S) has the same boundary as ranget; to fix: change width0 in CoastTileMain;
...
for j=1:length(idregion)
 ...
 % get the reg.txt
        infile= strrep([demdir,'/',f{i}],'meta.txt','reg.txt');
        if exist(infile)
        c=textread(infile,'%s','delimiter','\n');
...
%         df=p+dx4-dzxyd(j,:); %validation z, x, y
	...
	%write translation parameters
        fid = fopen('output/pxyz.dat','a'); %statistics
	fprintf(fid,' %f %f %f (px, py, pz; strip:) %s \n',p(2),p(3),p(1),infile);
	fclose(fid);
...
dx=100;%800;%(suggest 800);%400;%3600;%7e3;%%17e3; %3600; %80; %3600; 
dxov=dx/4;%200;
...
%     	[~,idsort]=sort(t);id=id(idsort); %sort the id based on tim
	...
	if isempty(datarsv(iisv).x) % non exist
        ...
        clear data
        tic
        data=multispec(infile,wm); %given strip meta file, finding all image 1 multispectral imageries, orthrectiying, get water mask.
...
%save work1.mat demg
	...
	else %method 2 ; Use the water probability, tend to reserve more data in clouds detection.
...
%save sv5.mat xout yout iselop novlpf novlpfc prob jump -v7.3
ck2=clock;
...
%Fix Bug 6: old method: treat the edge (no data) as land, which separate water bodies and treating some ocean as lakes and ignored in bwboundaries.
% New method: treat the edge as ocean.-> produce new problems:produces some wrong coastline by some small lakes along the edge.
% So ignore this problem for now. Still use the old method.
Modj=jump;Modj(Medgs)=0;%edges as land,but would be filterd out again later.
%Modj=jump;Modj(Medgs)=1;%edges as ocean,but would be filterd out again later.
...
%remove the buffer width of the tile box;
idx=xout<rang0b(1)|xout>rang0b(2);idy=yout<rang0b(3)|yout>rang0b(4);
Medgstb(:,idx)=1;Medgstb(idy,:)=1;

Medgs=Medgs|(prob==255|novlpf<=1); %add more edge based on repeats.
Medgs1=imdilate(Medgs,ones(3))|Medgstb;
B = bwboundaries(~Modfil,'noholes'); %^_^
